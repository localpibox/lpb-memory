...
/**
 * Memory extension file logger.
 * All extension activity (consolidation, review, flush, correction, indexing)
 * is logged here for observability. Rotated at 5MB.
 */
...
const LOG_FILE = (): string => path.join(logDir, "memory-extension.log");

let logFd: number | null = null;

/**
 * Redirect the extension log to a custom directory (e.g. the configured
 * memoryDir). Safe to call any time before logging happens; pending fd is
 * closed and reopened on the next write. Falls back to the default dir if
 * never called.
 */
...
function rotateIfNeeded(): void {
  try {
    if (logFd !== null && fs.existsSync(LOG_FILE())) {
...
export function logMemory(message: string, level: "info" | "warn" | "error" = "info"): void {
  try {
    rotateIfNeeded();
    if (logFd === null) {
      try {
...
/** Flush and close the log file (called during shutdown). */
