...
export function loadConfig(configPath = DEFAULT_CONFIG_PATH): MemoryConfig {
  try {
    if (fs.existsSync(configPath)) {
      ...
      if (typeof parsed.reviewTimeoutMs === "number") config.reviewTimeoutMs = parsed.reviewTimeoutMs;
    ...
    // Fall back to defaults on parse error or access issues — warn the user.
    const msg = err instanceof Error ? err.message : String(err);
    logMemory(`Config parse error at ${configPath}: ${msg}. Falling back to defaults.`, "warn");
  }
