...
import { scheduleSessionBackfill, waitForSessionBackfill, SESSION_BACKFILL_SHUTDOWN_TIMEOUT_MS, resetSessionBackfillState } from "./handlers/session-backfill.js";
import { scheduleLiveSessionIndex, waitForLiveSessionIndex, SESSION_LIVE_INDEX_SHUTDOWN_TIMEOUT_MS, resetSessionLiveIndexState } from "./handlers/session-live-index.js";
...
import { closeLog, setLogDir } from "./constants.js";
...
export default function (pi: ExtensionAPI) {
  ...
  // Keep the extension log colocated with the active memory directory so a
  // configurable memoryDir also relocates observability output.
  setLogDir(globalDir);
  ...
  // Track the current model for use by auto-consolidator and consolidation subprocess
  let currentModelRef: { provider: string; id: string } | null = null;
  ...
  // Build shared config with model override for all LLM subprocess paths.
  ...
  pi.on("session_start", async (_event, ctx) => {
    // Reset shared module state for new session — prevents cross-session leakage.
    resetSessionBackfillState();
    resetSessionLiveIndexState();

    // Store current model for use by consolidation/LLM subprocesses.
    // Skip "unknown/unknown" placeholders that appear before the provider
    // is fully loaded — using them would break subprocesses that inherit
    // the model via llmModelOverride.
    if (ctx.model && ctx.model.provider !== "unknown" && ctx.model.id !== "unknown") {
      currentModelRef = { provider: ctx.model.provider, id: ctx.model.id };
    }
  ...
  store.setConsolidator(async (target, signal) => {
    return triggerConsolidation(pi, store, target, signal, config.consolidationTimeoutMs, target, getSharedConfig());
  });
  if (projectStore) {
    projectStore.setConsolidator(async (target, signal) => {
      const toolTarget = target === "memory" ? "project" : target;
      return triggerConsolidation(pi, projectStore, target, signal, config.consolidationTimeoutMs, toolTarget, getSharedConfig());
    });
  }
  registerConsolidateCommand(pi, store, config.consolidationTimeoutMs, projectStore, projectName, getSharedConfig);

  // ── 8. Setup correction detection ──
  setupCorrectionDetector(pi, store, projectStore, getSharedConfig, dbManager, projectName);
