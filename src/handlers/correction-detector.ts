...
import { getMessageText, resolveConfig } from "../types.js";
...
export function setupCorrectionDetector(
  ...
  if (!resolveConfig(config).correctionDetection) return;
  ...
  pi.on("message_end", async (event, _ctx) => {
    ...
    if (!text) return;
    if (isCorrection(text, resolveConfig(config))) {
      logMemory(`correctionDetected: pattern matched on user message: "${text.slice(0, 100)}"`);
  ...
  pi.on("turn_end", async (event, ctx) => {
    ...
    try {
      ...
      const runSubprocessCorrection = async (): Promise<void> => {
        // Guard: check if the session ctx is still accessible before spawning
        // a subprocess. If ctx was invalidated (session replacement), the
        // exec call would throw "stale ctx".
        try { ctx.sessionManager.getBranch(); } catch {
          logMemory(`correctionDetected: session ctx unavailable (likely replaced), skipping subprocess correction`, "warn");
          return;
        }

        logMemory(`correctionDetected: executing subprocess correction`);
        const subprocessPrompt = [CORRECTION_SAVE_PROMPT, "", ...promptBody].join("\n");
        // Disable thinking — correction save is a focused, short-lived task
        const correctionConfig = { ...cfg };
        const result = await execChildPrompt(pi, subprocessPrompt, correctionConfig, {
          signal: ctx.signal,
          timeoutMs: 30000,
        });
        if (result.code === 0 && result.stdout) {
          ...
          logMemory(`correctionDetected: subprocess result savedViaLlm=${savedViaLlm}`);
        }
      };
      ...
      if (usesDirectTransport(cfg)) {
        try {
          // Disable thinking for direct correction save — prevents overflow
          // when the prompt includes conversation context.
          const correctionConfig = { ...cfg };
          logMemory(`correctionDetected: attempting direct transport correction`);
          ...
          logMemory(`correctionDetected: direct transport result ok=${directResult.ok}, applied=${directResult.appliedCount}`);
          if (directResult.ok) {
          ...
          logMemory(`correctionDetected: direct transport failed, falling back to subprocess: ${errMsg}`, "warn");
      ...
      try {
        ...
        // Best-effort — don't block the session
        const errMsg = err instanceof Error ? err.message : String(err);
        logMemory(`correctionDetected: error in turn_end handler: ${errMsg}`, "error");
      } finally {
