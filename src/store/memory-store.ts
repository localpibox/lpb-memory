...
const RECOVERY_ACTIVE_GRACE_MS = 1 * 60 * 60 * 1000; // 1 hour (was 7 days) — consolidation storms create many stale recovery files
const RETIRED_RECOVERY_MAX_AGE_MS = 24 * 60 * 60 * 1000; // 1 day (was 30 days)
...
export class MemoryStore {
  ...
  private async pruneRecoveryFiles(filePath: string): Promise<void> {
    ...
    try {
      ...
      // Log at warn level so we know recovery cleanup is failing, but don't
      // block the main operation — disk-full or permission errors are
      // best-effort and shouldn't crash the extension.
      const errMsg = err instanceof Error ? err.message : String(err);
      logMemory(`pruneRecoveryFiles: failed for ${filePath}: ${errMsg}`, "warn");
    }
  }
