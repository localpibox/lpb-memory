...
const DEFAULT_RECOVERY_OPTIONS: ResolvedDatabaseRecoveryOptions = {
  ...
  recoveryLockStaleMs: 60000,  // 1 minute — aligns with markdown-mutation-lock staleMs
  recoveryCircuitLimit: 5,        // 5 failures — transient I/O hiccups are common,
  recoveryCircuitWindowMs: 600000, // 10 minute window — avoids bricking on brief storms
...
export class DatabaseManager {
  ...
  // Depth counter for recovery recursion — prevents stack overflow on persistently corrupt DB.
  private recoveryDepth = 0;
  private readonly maxRecoveryDepth = 3;
   ...
   */
  withCorruptionRecovery<T>(operation: () => T, depth = 0): T {
    try {
      return operation();
    } catch (err) {
      ...
      if (depth >= this.maxRecoveryDepth) {
        throw new Error(
          `SQLite recovery failed after ${depth} attempts for ${this.displayDbPath}. ` +
          `Database may be irrecoverably corrupt — recovery circuit should have blocked this.`,
        );
      }
      this.recoverFromCorruption(err);
      return this.withCorruptionRecovery(operation, depth + 1);
    }
  }
  ...
  /**
   * Ensure the recovery state file exists with a valid empty array.
   * If the file is missing (crash/kill orphan), we don't want the circuit
   * to silently disable itself — returning [] would mean 0 failures.
   * Instead we write an empty file so the circuit stays in effect.
   */
  private ensureRecoveryStateFile(): void {
    const statePath = this.recoveryCircuitPath();
    if (!fs.existsSync(statePath)) {
      try {
        fs.writeFileSync(statePath, JSON.stringify({ failures: [] }), { encoding: 'utf-8', mode: 0o600 });
      } catch {
        // Best-effort: if we can't write the file, the circuit will
        // silently see [] and stay closed. Not ideal but safe.
      }
    }
  }

  private recentRecoveryFailures(): number[] {
    // Ensure file exists first — prevents silent circuit disable after crash.
    this.ensureRecoveryStateFile();
    try {
  ...
  private backfillSessionsProject(db: DatabaseLike): void {
    ...
    // Build per-row project values for the CASE expression.
    const cases: string[] = [];
    const params: (string | number)[] = [];
    for (const row of projectCwd) {
