...
/** A value that can be either a config object or a zero-arg function that produces one. */
export type ConfigOrProvider<T> = T | (() => T);

/** Resolve a config-or-provider to a concrete config object. If T is a function, call it; otherwise return as-is. */
export function resolveConfig<T>(config: ConfigOrProvider<T>): T {
  return typeof config === "function" ? (config as () => T)() : config;
}
